...
if(_insideMarker)then{_roads=civTraffic_roadSegments select _index;_roads pushBack _road;
	...
	if(!(civTraffic_edgeRoadsUseful select _index))then{civTraffic_edgeRoadsUseful set[_index,true];};
sleep 0.01;};
_index=_index+1;};
...
_this,"cars",["CUP_C_Datsun_Tubeframe","CUP_C_Datsun","CUP_C_Datsun_Plain","walker_a3_datsun","CUP_C_Skoda_White_CIV","CUP_C_Skoda_Blue_CIV","CUP_C_Ural_Civ_01","CUP_C_Ural_Open_Civ_01","walker_a3_hilux","walker_a3_v_hilux","walker_a3_gaz","walker_a3_gazblue","walker_a3_lada"]]call civTraffic_GetParamValue;};
default{_carList=[
...
sleep 0.01;
{_x enableSimulation false;_x disableAI"ALL";_x hideObjectGlobal true;[_x]execVM "eos\fn\randOP4.sqf";_x forceSpeed 10;
sleep 0.01;}forEach _carsCrew;

//50% chance for random passengers
if(round(random 2)==1)then{
private _seats=_car emptyPositions"cargo";
if(_seats>1)then{
private _seats=floor random _seats;
if(_seats>11)then{_seats=11};
for "_x" from 0 to _seats do {
private _unit=createAgent["C_Man_1",[0,0,100],[],0,""];_unit moveInCargo _car;
removeGoggles _unit;removeHeadgear _unit;removeAllAssignedItems _unit;[_unit]call InsG;[_unit]call InsH;[_unit]call InsU;};}else{
if(round(random 2)==1)then{
private _unit=createAgent["C_Man_1",[0,0,100],[],0,""];_unit moveInCargo _car;
removeGoggles _unit;removeHeadgear _unit;removeAllAssignedItems _unit;[_unit]call InsG;[_unit]call InsH;[_unit]call InsU;};};
};
...
//Delete crew
//{deleteVehicle _x;}forEach _crewUnits;//OLD
{_car deleteVehicleCrew _x}forEach crew _car;//New (My shit)
