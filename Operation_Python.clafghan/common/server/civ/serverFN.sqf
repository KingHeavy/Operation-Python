...
if(markerShape _markerName=="ELLIPSE")then{_res=(((_rpx-_mpx)^2)/(_msx^2))+(((_rpy-_mpy)^2)/(_msy^2));
...
_playerPositions=[];
if(isMultiplayer)then{{if(isPlayer _x)then{_playerPositions pushBack (position vehicle _x);};}forEach(playableUnits);}else{
	if(player==player)then{_playerPositions=[position vehicle player];};};
_playerPositions
};
...
_count=0;
while{format["%1",_building buildingPos _count]!="[0,0,0]"}do{_count=_count+1;};
_count
};
...
_tooClose=false;
	...
	if(time>5)then{{if(_x distance _building<_minSpawnDistance)then{_tooClose=true;};}forEach _playerPositions;};
	if(!_tooClose)then{
...
if(_buildingPosCount>0)then{
	...
	_playerBuildingsTemp pushBack[_x,_buildingPosCount];};};}forEach _playerBuildings;		
	_playerBuildingsTemp
};
...
if(_unitsCount>_maxGroupsCount)then{_unitsCount=_maxGroupsCount;};
...
_tooClose=false;
{if(_x distance _civilian<_maxSpawnDistance)then{_tooClose=true;};}forEach _allPlayerPositions;
	if(_tooClose)then{_civilianItemsTemp pushBack _x;}else{
	_removeUnit=[_civilian]call _fnc_OnRemoveCallback;
		if(isNil "_removeUnit")then{_removeUnit=true;};
		if(typeName _removeUnit!="BOOL")then{_removeUnit=true;};
		if(!_removeUnit)then{_civilianItemsTemp pushBack _x;}else{
